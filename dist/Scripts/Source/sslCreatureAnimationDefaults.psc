...
; 本文件只用于描述 legacy sslCreatureAnimationDefaults 在 SLNG 中的迁移方案，不提供运行时实现。
; 当前总策略:
; - 注册统一在 SKSE 侧实现。
...
;
; File role:
; - Built-in creature animation content list grouped by creature category.

; Main entry
; - LoadCreatureAnimations()
; Purpose: prepare the creature factory, check legacy creature support flags, register creature animations, and register category names.
; Params: none.

; Why this file should not be native
; - This is content definition, not backend logic.